...
class CommentForm extends Component {
  ...
  renderComments = (comment, index) => {
    const { removeComment, deleteUser, postId, user } = this.props;
            ...
            {user.role === 'Administrator' ? (
              <span>
                <button
                  className="username"
                  onClick={() => {
                    deleteUser(comment.user);
                  }}
                >
                  <strong>
                    {' '}
                    <Avatar /> {`delete ${comment.user}'s account`}{' '}
                  </strong>
                </button>
                <button
                  className="username"
                  onClick={() => {
                    this.props.actions.changeUserPrivileges(comment.user);
                  }}
                >{`demote/promote ${comment.user}`}</button>
              </span>
            ) : (
              <strong>
                <Avatar /> {comment.user}:
  ...
  render() {
    const { url, comments, user } = this.props;

    return (
      <div>
        <span>
          <Button className="mx-0" color="success" outline size="sm" href={url}>
            Link to article
          </Button>
...
export default connect(
  storeConfiguration.mapStateToProps,
  storeConfiguration.mapDispatchToProps
)(CommentForm);
